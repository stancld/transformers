...
class DebertaModelTester(object):
    ...
    def create_and_check_deberta_for_causal_lm(
        self,
        config,
        input_ids,
        token_type_ids,
        ...
        model = DebertaLMHeadModel(config=config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_deberta_for_causal_lm_as_decoder(
        self,
        config,
        input_ids,
        token_type_ids,
    ...
    def create_and_check_decoder_deberta_past_large_inputs(
        self,
        config,
        input_ids,
        token_type_ids,
        ...
        config.is_decoder = True
        config.add_cross_attention = True
        model = DebertaLMHeadModel(config=config).to(torch_device).eval()
...
class DebertaModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    all_generative_model_classes = (DebertaLMHeadModel,) if is_torch_available() else ()
    ...
    def test_deberta_as_decoder(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs_for_decoder()
        self.model_tester.create_and_check_model_as_decoder(*config_and_inputs)

    def test_deberta_as_decoder_with_default_input_mask(self):
