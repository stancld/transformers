# coding=utf-8
# Copyright 2021 BART Authors (Fairseq Authors), I-BERT Authors (Sehoon Kim, Amir Gholami, Zhewei Yao, Michael Mahoney, Kurt Keutzer - UC Berkeley), The HuggingFace Inc. team and Daniel Stancl The HuggingFace Inc. team. All rights reserved.
#
...
class IBartLearnedPositionalEmbedding(nn.Embedding):
    """
...
class IBartAttention(nn.Module):
...
class IBartClassificationHead(nn.Module):
...
IBART_GENERATION_EXAMPLE = r"""
    Summarization example::
        ...
        >>> from transformers import IBartTokenizer, IBartForConditionalGeneration, IBartConfig

        >>> model = IBartForConditionalGeneration.from_pretrained('brand-new-bert-base-cased')
        >>> tokenizer = IBartTokenizer.from_pretrained('brand-new-bert-base-cased')
...
IBART_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (:obj:`torch.LongTensor` of shape :obj:`(batch_size, sequence_length)`):
            Indices of input sequence tokens in the vocabulary. Padding will be ignored by default should you provide
            it.

            Indices can be obtained using :class:`~transformers.IBartTokenizer`. See
        ...
        decoder_attention_mask (:obj:`torch.LongTensor` of shape :obj:`(batch_size, target_sequence_length)`, `optional`):
            ...
            If you want to change padding behavior, you should read :func:`modeling_ibart._prepare_decoder_inputs` and
...
IBART_STANDALONE_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (:obj:`torch.LongTensor` of shape :obj:`(batch_size, sequence_length)`):
            Indices of input sequence tokens in the vocabulary. Padding will be ignored by default should you provide
            it.
...
class IBartEncoder(IBartPreTrainedModel):
    """
    Transformer encoder consisting of *config.encoder_layers* self attention layers. Each layer is a
    :class:`IBartEncoderLayer`.
...
class IBartDecoder(IBartPreTrainedModel):
    """
    Transformer decoder consisting of *config.decoder_layers* layers. Each layer is a :class:`IBartDecoderLayer`
...
@add_start_docstrings(
    "The bare IBART Model outputting raw hidden-states without any specific head on top.",
    IBART_START_DOCSTRING,
)
class IBartModel(IBartPreTrainedModel):
    def __init__(self, config: IBartConfig):
        super().__init__(config)
    ...
    @add_start_docstrings_to_model_forward(IBART_INPUTS_DOCSTRING)
    @add_code_sample_docstrings(
...
@add_start_docstrings(
    "The IBART Model with a language modeling head. Can be used for summarization.", IBART_START_DOCSTRING
)
class IBartForConditionalGeneration(IBartPreTrainedModel):
    base_model_prefix = "model"
    ...
    @add_end_docstrings(IBART_GENERATION_EXAMPLE)
    def forward(
        self,
        input_ids=None,
        attention_mask=None,
        ...
        Conditional generation example::
            ...
            >>> from transformers import IBartTokenizer, IBartForConditionalGeneration
            >>> tokenizer = IBartTokenizer.from_pretrained('brand-new-bert-base-cased')
...
@add_start_docstrings(
    """
    IBart model with a sequence classification/head on top (a linear layer on top of the pooled output) e.g. for GLUE
    tasks.
    """,
    IBART_START_DOCSTRING,
)
class IBartForSequenceClassification(IBartPreTrainedModel):
    def __init__(self, config: IBartConfig, **kwargs):
        super().__init__(config, **kwargs)
        self.model = IBartModel(config)
        self.classification_head = IBartClassificationHead(
            config.d_model,
            config.d_model,
            config.num_labels,
...
@add_start_docstrings(
    """
    IBART Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear
    layer on top of the hidden-states output to compute `span start logits` and `span end logits`).
    """,
    IBART_START_DOCSTRING,
)
class IBartForQuestionAnswering(IBartPreTrainedModel):
...
class IBartDecoderWrapper(IBartPreTrainedModel):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.decoder = IBartDecoder(config)

    def forward(self, *args, **kwargs):
        return self.decoder(*args, **kwargs)


class IBartForCausalLM(IBartPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        config = copy.deepcopy(config)
        config.is_decoder = True
        config.is_encoder_decoder = False
        self.model = IBartDecoderWrapper(config)
    ...
    def forward(
        ...
        Example::
            ...
            >>> from transformers import IBartTokenizer, IBartForCausalLM

            >>> tokenizer = IBartTokenizer.from_pretrained('facebook/bart-large')
            >>> model = IBartForCausalLM.from_pretrained('facebook/bart-large', add_cross_attention=False)
