...
class DisentangledSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if is_cross_attention and query_states is not None:
            ...
        elif is_cross_attention:
            qp = self.in_proj(encoder_hidden_states)  # .split(self.all_head_size, dim=-1)
            query_layer, key_layer, value_layer = self.transpose_for_scores(qp).chunk(3, dim=-1)
            attention_mask = encoder_attention_mask
        elif query_states is None:
...
@add_start_docstrings(
    """DeBERTa Model with a `language modeling` head on top for CLM fine-tuning. """, DEBERTA_START_DOCSTRING
)
class DebertaLMHeadModel(DebertaPreTrainedModel):
    ...
    def forward(
        ...
        Example::
            ...
            >>> from transformers import DebertaTokenizer, DebertaLMHeadModel, DebertaConfig
            >>> import torch

            >>> tokenizer = DebertaTokenizer.from_pretrained('microsoft/deberta-base')
            >>> config = DebertaConfig.from_pretrained("microsoft/deberta-base")
            >>> config.is_decoder = True
            >>> model = DebertaLMHeadModel.from_pretrained('microsoft/deberta-base', config=config)
