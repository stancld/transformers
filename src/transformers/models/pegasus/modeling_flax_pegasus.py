...
class FlaxPegasusPretrainedModel(FlaxPreTrainedModel):
    ...
    def encode(
        ...
        Example::
            ...
            >>> model = FlaxPegasusForConditionalGeneration.from_pretrained('facebook/google/pegasus-large')
            >>> tokenizer = PegasusTokenizer.from_pretrained('facebook/google/pegasus-large')
...
class FlaxPegasusForSequenceClassificationModule(nn.Module):
    config: PegasusConfig
    dtype: jnp.dtype = jnp.float32
    ...
    def setup(self):
        self.model = FlaxPegasusModule(config=self.config, dtype=self.dtype)
        self.classification_head = FlaxPegasusClassificationHead(
            config=self.config,
...
@add_start_docstrings(
    """
    Pegasus model with a sequence classification/head on top (a linear layer on top of the pooled output) e.g. for GLUE
    tasks.
    """,
    PEGASUS_START_DOCSTRING,
)
class FlaxPegasusForSequenceClassification(FlaxPegasusPretrainedModel):
    module_class = FlaxPegasusForSequenceClassificationModule
    dtype = jnp.float32


append_call_sample_docstring(
    FlaxPegasusForSequenceClassification,
    _TOKENIZER_FOR_DOC,
    _CHECKPOINT_FOR_DOC,
    FlaxSeq2SeqSequenceClassifierOutput,
    _CONFIG_FOR_DOC,
...
class FlaxPegasusForQuestionAnsweringModule(nn.Module):
    config: PegasusConfig
    dtype: jnp.dtype = jnp.float32
    num_labels = 2
...
@add_start_docstrings(
    """
    Pegasus Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear
    layer on top of the hidden-states output to compute `span start logits` and `span end logits`).
    """,
    PEGASUS_START_DOCSTRING,
)
class FlaxPegasusForQuestionAnswering(FlaxPegasusPretrainedModel):
    module_class = FlaxPegasusForQuestionAnsweringModule
    dtype = jnp.float32


append_call_sample_docstring(
    FlaxPegasusForQuestionAnswering,
    _TOKENIZER_FOR_DOC,
    _CHECKPOINT_FOR_DOC,
    FlaxSeq2SeqQuestionAnsweringModelOutput,
    _CONFIG_FOR_DOC,
